...
import { Clock, Star, Zap, CheckCircle2, XCircle, Trophy, RotateCcw, Play, BookOpen } from 'lucide-react';
...
const SpellingChallenge = () => {
  ...
  const startGame = (selectedDifficulty = 'easy') => {
    ...
    setCurrentWord(0);
    setTimeLeft(selectedDifficulty === 'easy' ? 90 : selectedDifficulty === 'medium' ? 120 : 150);
    setScore(0);
    setStreak(0);
    setUserLetters([]);
    setFeedback(null);
  ...
  const submitAnswer = () => {
    ...
    if (userWord === correctWord) {
      ...
      const bonusTime = difficulty === 'easy' ? 10 : difficulty === 'medium' ? 15 : 20;
      setTimeLeft(prev => Math.min(prev + bonusTime, difficulty === 'easy' ? 90 : difficulty === 'medium' ? 120 : 150));
      
      const basePoints = difficulty === 'easy' ? 10 : difficulty === 'medium' ? 20 : 30;
      const points = streak >= 3 ? basePoints * 2 : basePoints;
      setScore(prev => prev + points);
      setStreak(prev => prev + 1);
      setFeedback({ type: 'success', message: `Correct! +${points} points +${bonusTime} seconds` });
      ...
      // Wrong answer
      setStreak(0);
      setFeedback({ type: 'error', message: `Incorrect. The answer is "${correctWord}"` });
      setTimeout(() => {
        setUserLetters([]);
        setFeedback(null);
      }, 3000);
  ...
  const handleKeyPress = (e, index) => {
    if (e.key === 'Enter') {
      if (index === currentWordData.word.length - 1 && userLetters.filter(l => l).length === currentWordData.word.length) {
        submitAnswer();
      }
  ...
  const getDifficultyBg = (diff) => {
    switch(diff) {
      case 'easy': return 'bg-green-500/20 border-green-400/50';
      case 'medium': return 'bg-yellow-500/20 border-yellow-400/50';
      case 'hard': return 'bg-red-500/20 border-red-400/50';
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 flex items-center justify-center p-4">
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="mb-8">
            ...
            <h1 className="text-4xl font-bold text-white mb-2">Word Spelling</h1>
            <p className="text-purple-200">Type each letter to spell the word correctly!</p>
          </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 flex items-center justify-center p-4">
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
          <div className="mb-8">
            ...
            <p className="text-purple-200">Well done on the {difficulty} challenge!</p>
          </div>
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="backdrop-blur-sm bg-white/5 rounded-2xl p-4 border border-white/10">
              <div className="text-2xl font-bold text-white mb-1">{currentWord + 1}/{currentWords.length}</div>
              <div className="text-purple-200">Words Completed</div>
            </div>
            <div className={`backdrop-blur-sm rounded-2xl p-4 border ${getDifficultyBg(difficulty)}`}>
              <div className={`text-xl font-bold ${getDifficultyColor(difficulty)} mb-1 capitalize`}>{difficulty}</div>
  ...
  const maxTime = difficulty === 'easy' ? 90 : difficulty === 'medium' ? 120 : 150;
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-800 to-indigo-900 p-4">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-2xl font-bold ${timeLeft <= 15 ? 'text-red-400' : 'text-white'}`}>
                  {timeLeft}s
                </span>
              </div>
              <div className="flex items-center gap-2">
              ...
              {streak > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-bold text-orange-400">{streak}x</span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-4">
              <div className={`px-3 py-1 rounded-full text-sm font-bold capitalize backdrop-blur-sm border ${getDifficultyBg(difficulty)} ${getDifficultyColor(difficulty)}`}>
                {difficulty}
              </div>
        ...
        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl">
          <div key={animationKey} className="animate-in slide-in-from-left duration-500">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-white mb-2">Spell this word:</h2>
              <div className="text-lg text-purple-200 mb-6">{currentWordData.hint}</div>
              ...
              <button
                ...
                disabled={userLetters.filter(l => l).length !== currentWordData.word.length}
          ...
          <div className="backdrop-blur-sm bg-white/5 rounded-2xl p-6 border border-white/10 mt-8">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-purple-100 text-lg leading-relaxed">{currentWordData.definition}</p>
              </div>
            </div>
          </div>
          ...
          {/* Feedback */}
          {feedback && (
            <div className={`mt-6 p-4 rounded-2xl backdrop-blur-sm border animate-in slide-in-from-bottom duration-300 ${
