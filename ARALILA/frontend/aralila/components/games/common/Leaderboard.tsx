  ...
  return (
    <div className="w-full bg-white/5 backdrop-blur-sm rounded-2xl p-4 border border-white/8">
      ...
      <ul className="space-y-3">
        {entries.map((e, idx) => (
          <li key={`${e.user_id}-${idx}`} className="flex items-center gap-3 bg-white/5 p-2 rounded-lg">
            <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 flex-shrink-0">
              {e.profile_pic ? (
                <Image src={e.profile_pic} alt={e.name} width={48} height={48} className="object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-sm text-gray-600">{e.name?.charAt(0) || "U"}</div>
              )}
            </div>
        ...
        {entries.length === 0 && !loading && <li className="text-sm text-gray-400">No entries yet.</li>}
      </ul>
    </div>
  );
};
