...
const buildCorrectSentence = (result: PunctuationResult) => {
    ...
    sortedPunctuation.forEach(p => {
        if(p.position === -1) { // -1 typically means add to the end of the sentence
            sentence += p.mark;
        } else {
...
export const PunctuationChallengeSummary = ({ score, results }: SummaryProps) => {
  ...
  if (accuracy === 100) {
    summaryContent = { title: "Perfect Lesson!", imageSrc: "/images/character/lila-happy.png", showConfetti: true };
  } else if (accuracy >= 80) {
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                      <p className="text-xl font-bold text-slate-800">{result.sentenceData.sentence}</p>
                      {!result.isCorrect && (
                        <p className="text-md text-red-700 bg-red-100 px-2 py-1 rounded-md mt-1 sm:mt-0">
                          Your answer: <span className="font-mono font-bold">{result.userAnswer || '""'}</span>
                        </p>
                      )}
                    </div>
                    <p className="text-slate-600 mt-1">Correct: <span className="font-mono font-bold">{buildCorrectSentence(result)}</span></p>
                  </div>
                </motion.div>
              ))}
            </div>
