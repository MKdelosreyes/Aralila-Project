...
const AssignmentCreationModal = ({ isOpen, onClose, asDrawer }) => {
  ...
  const handleCreate = async () => {
    ...
    console.log(numericClassId);
    setClassroomId(Number(classId));
    ...
    formData.append("classroom_id", numericClassId.toString());
    ...
    try {
      const response = await activityAPI.createClassroomActivity(
        numericClassId,
        formData
      );
              ...
              <div className="flex gap-4">
                <div className="flex flex-col">
                  ...
                  <input
                    type="number"
                    min="0"
                    value={durationHours.toString()} // Cast to string to avoid NaN warning
                    onChange={(e) => {
                      const value = parseInt(e.target.value) || 0; // Default to 0 if NaN
                      setDurationHours(value);
                    }}
                    className="h-10 w-24 rounded-xl border border-input bg-background px-3 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 transition-all"
                  />
                </div>
                <div className="flex flex-col">
                  <label className="text-xs text-muted-foreground mb-1">
                  ...
                  <input
                    ...
                    min="0"
                    max="59"
                    value={durationMinutes.toString()} // Cast to string to avoid NaN warning
                    onChange={(e) => {
                      const value = parseInt(e.target.value) || 0; // Default to 0 if NaN
                      // Ensure value does not exceed max
                      setDurationMinutes(Math.min(Math.max(value, 0), 59));
                    }}
                    className="h-10 w-24 rounded-xl border border-input bg-background px-3 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 transition-all"
                  />
                </div>
